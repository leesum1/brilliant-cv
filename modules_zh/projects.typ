...
#cvEntry(
  ...
  location: [一生一芯是涉及处理器设计、系统软件开发、SOC 构建的综合性实践项目],
  description: list(
    [*处理器设计与实现：*使用Verilog设计RV64IM指令集的经典5级流水线处理器，并使用C语言开发RISC-V模拟器，增强了对硬件架构的理解],
    [*软件移植与系统调试：*从系统底层为处理器移植软件，熟练掌握交叉编译、链接脚本、操作系统移植、GDB调试和Makefile脚本，提升软件与硬件的协同工作能力],
    [完成*「一生一芯-第四期」*所有内容，使用 *AXI4 总线*将处理器接入到 「ysyx-SOC」 环境，通过了 riscv-tests 等测试],
  ),
)
...
//   description: list(
//     [*熟悉 C/C++、RUST 语言*，使用 RUST 编写了支持 RV64IMAC 的模拟器，添加了 ICahce, DCache 等缓存],
...
#cvEntry(
  ...
  description: list(
    [*熟悉 C/C++、RUST 语言：*使用 RUST 编写了支持 RV64IMAC 的模拟器，添加了 ICahce, DCache 等缓存],
    [*底层驱动开发：*构建了完善的设备框架，成功集成了 PLIC 中断控制器、VGA、UART等多种外设，并实现了对应的驱动移植],
    [*系统软件移植：*独立完成 RT-Thread、Linux 等操作系统的移植工作，验证模拟器的功能正确性],
    [*调试技术实现：*深入理解处理器调试技术，为模拟器实现Debug模式，通过 openOCD/GDB 与 SimJTAG 连接，支持单步调试、寄存器和内存的读写操作],
  ),
)
...
#cvEntry(
  ...
  description: list(
    // [熟悉*计算机体系架构*，对多级*分支预测*、*乱序执行*等内容有深入了解，使用 Chisel 实现了*乱序双发射 7 级流水线*的 RISC-V 处理器设计，能够运行*Linux*、CoreMark 等复杂应用],
    // [熟悉*计算机体系架构*，使用 Chisel 实现了*乱序双发射 7 级流水线*的 RISC-V 处理器设计，能够运行*Linux*、CoreMark 等复杂应用],
    [*计算机体系架构与处理器设计：* 熟悉计算机体系架构，使用 Chisel 实现乱序双发射 7 级流水线 RISC-V 处理器设计，成功运行Linux和CoreMark等复杂应用程序],
    [*SOC架构与总线协议：* 设计并验证 AXI4xbar、AXI4Arbiter、多端口 FIFO 和跨时钟握手协议等模块，构建完整的 FishSOC 系统并进行仿真与验证],
    [*RISC-V 调试机制：*，实现了 *JtagDMI* 与 *DebugModule*，能够通过 *JTAG* 连接到 FishSOC 进行调试操作],
    ...
    // [熟悉*AMBA等总线协议*，对*SOC芯片架构*有一定的理解，实现了*AXI4 总线桥、AXI4Demux、AXI4Arbiter、多端口 FIFO、跨时钟握手协议*等多种模块；从零开始为 FishCore
    ...
    [*FPGA 开发经验：*使用 Vivado 对 FishSOC 进行综合、布局布线，成功在 FPGA 运行了 benchmark 等测试程序],
    [*熟练使用各种 EDA 工具：*例如 *Vivado、VCS、Verdi、Iverilog、Verilator* 等],
  ),
)
...
#cvEntry(
  ...
  description: list(
    [#link("https://github.com/RT-Thread/rt-thread/pull/7040")[ *#fa-icon("code-merge") rt-thread/pull/7040* ]：修复
      qemu-riscv-virt64 bsp start.s 中错误的内存分配问题，能够在 smode 下成功启动系统],
